import React from "react";
import { Calendar, CreditCard, Check } from "lucide-react";

const ProgressIndicator = ({ currentStep }) => {
  const steps = [
    {
      id: 1,
      title: "Select Date & Time",
      icon: Calendar,
      description: "Select date, time and booking information",
    },
    {
      id: 2,
      title: "Payment",
      icon: CreditCard,
      description: "Online banking payment",
    },
    {
      id: 3,
      title: "Complete",
      icon: Check,
      description: "Booking confirmed successfully",
    },
  ];

  return (
    <div className="bg-gradient-to-r from-gray-50 via-slate-50 to-gray-100 border-b border-gray-200 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 py-4">
        <div className="flex items-center justify-between">
          {steps.map((step, index) => (
            <div key={step.id} className="flex items-center">
              <div
                className={`flex items-center justify-center aspect-square w-10 rounded-full transition-all duration-300 ${
                  currentStep >= step.id
                    ? "bg-blue-600 text-white shadow-md"
                    : "bg-white text-gray-400 border-2 border-gray-300"
                }`}
              >
                <step.icon className="w-5 h-5" />
              </div>
              <div className="ml-3">
                <div
                  className={`text-sm font-semibold transition-colors ${
                    currentStep >= step.id ? "text-blue-600" : "text-gray-500"
                  }`}
                >
                  {step.title}
                </div>
                <div className="text-xs text-gray-500">{step.description}</div>
              </div>
              {index < steps.length - 1 && (
                <div
                  className={`w-12 h-0.5 mx-4 transition-colors ${
                    currentStep > step.id ? "bg-blue-600" : "bg-gray-300"
                  }`}
                />
              )}
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default ProgressIndicator;
